...
const PatientDetails = () => {
  ...
  const updateStatusMutation = useMutation({
    mutationFn: (status: PatientStatus) => {
      return patientService.update(id!, { status });
    },
    onSuccess: () => {
  ...
  const handleDeletePatient = () => {
    if (window.confirm("Tem certeza que deseja remover este paciente? Esta ação não pode ser desfeita.")) {
  ...
  const formattedCreatedAt = format(parseISO(patient.createdAt), "PPP", { locale: ptBR });
  const formattedUpdatedAt = format(parseISO(patient.updatedAt), "PPP", { locale: ptBR });
              ...
              <Dialog open={isStatusDialogOpen} onOpenChange={setIsStatusDialogOpen}>
                ...
                <DialogContent>
                  ...
                  <div className="py-4">
                    <RadioGroup
                      value={newStatus}
                      onValueChange={(value) => setNewStatus(value as PatientStatus)}
                      className="space-y-4"
                    >
                      <div className="flex items-center space-x-2">
                        <RadioGroupItem value="active" id="status-active" />
                        <Label htmlFor="status-active">Ativo (recebendo visitas)</Label>
                      </div>
                      <div className="flex items-center space-x-2">
                        <RadioGroupItem value="recovered" id="status-recovered" />
                        <Label htmlFor="status-recovered">Recuperado (não precisa mais de visitas)</Label>
                      </div>
                      <div className="flex items-center space-x-2">
